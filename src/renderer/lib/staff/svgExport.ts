...
import type { RenderBeamGroup, RenderBeamPoint, RenderEvent, RenderMeasure, RenderPart, RenderScore, RenderStaff, RenderSystem, RenderTuplet } from "./types";

const LINE_GAP = DEFAULT_RENDER_LAYOUT_OPTIONS.lineGap;

export function renderScoreToSvg(renderScore: RenderScore): string {
  const body = renderScore.systems.map((system) => staffSystemMarkup(renderScore, system)).join("");
