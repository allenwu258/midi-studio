...
- 切换模式后立即保存到 SQLite。
- 若当前已有 MIDI 曲目，提示“设置将在重新载入曲目后完全生效”，或者在保存后重建播放器并重新加载当前 MIDI。
...
4. 播放器工厂释放旧播放器并创建新播放器。
5. 如果已有曲目，重新 load 当前 MIDI bytes。
...
- 切换模式时先暂停并 dispose 旧播放器。
- 使用保存的原始 MIDI bytes 重新 load 新播放器。
- 重建后 seek 回切换前的位置。
- 如果 SF2 模式加载失败，停留在当前设置并提示错误，不偷偷改成另一种模式。
