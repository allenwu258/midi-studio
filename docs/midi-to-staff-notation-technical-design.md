...
MIDI 逆向五线谱与 MusicXML 导入/导出技术方案。
...
当前 `midi-studio` 已有 MIDI 解析、播放、五线谱主视图、MusicXML 导入和
alphaSynth + SF2 播放。下一阶段如果要继续提升五线谱质量并扩展 MusicXML 导出，
核心问题不是 XML 序列化，而是从 MIDI 演奏事件或 MusicXML 记谱语义中稳定提取可
渲染的乐谱结构：
...
`midi-studio` 可以先支持干净 MIDI 的五线谱，并把 MusicXML 导入与导出都挂在同一
套乐谱中间模型上，再逐步逼近 MuseScore 的复杂导入质量。
...
截至当前 `codex/musicxml-input` 分支，本文档中的核心 ScoreDraft 管线已经落地，
MusicXML 导入也已经完成，当前仍未实现的是 MusicXML 导出。
...
- `src/renderer/lib/musicxml/parseMusicXml.ts` 已支持 `.xml` / `.musicxml` / `.mxl`
  导入，并在独立 Worker 中完成解析。
- `src/renderer/lib/musicxml/toScoreDraft.ts` 直接把 MusicXML 语义建成 `ScoreDraft`，
  不再先经由通用 MIDI 重新量化再反推谱面。
- `src/renderer/lib/musicxml/toMidi.ts` 负责生成播放侧 MIDI bytes，确保 alphaSynth 仍
  走稳定的播放路径。
- `scripts/validate-musicxml-fixtures.mjs` 已建立 MusicXML fixture 校验，覆盖单声部、
  和弦、rest、backup/forward、多 voice、双谱表、tie、tempo、key/time change、
  tuplet 和 `.mxl`。
...
MusicXML 输入不应退化为“先生成 MIDI，再从 MIDI 重新猜谱”。当前实现先解析为
`MusicXmlScoreSource`，保留 MusicXML 自带的记谱语义，再分别进入谱面和播放路径：
...
MusicXML / .mxl
  ...
  -> MusicXmlScoreSource
  -> toScoreDraft()
  -> ScoreDraft / RenderScore / PlaybackMap
  -> toMidi() / buildMidiBytes()
```

关键文件：

```text
src/renderer/lib/musicxml/parseMusicXml.ts
src/renderer/lib/musicxml/toScoreDraft.ts
src/renderer/lib/musicxml/toMidi.ts
src/renderer/lib/musicxml/types.ts
src/renderer/workers/musicXmlParseWorker.ts
```
...
`MusicXmlScoreSource` 需要保留：

- 原始 voice / staff。
- note type / dots。
- tuplet / time-modification。
- tie start / stop 语义。
- measure attributes，包括 divisions、拍号、调号、谱号和 staves。
...
当前导入路径已经落地，导出仍应从 `ScoreDraft` 输出 `score-partwise`。
...
- 当前分支已经完成 MusicXML 导入基础，这一阶段后续主要补导出侧。
...
```text
src/renderer/lib/musicxml/exportMusicXml.ts
...
- MusicXML 导入可以生成可渲染的 ScoreDraft，并保留播放路径。
